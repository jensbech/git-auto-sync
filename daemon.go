...
func daemonStatus(ctx *cli.Context) error {
	...
	client, err := common.NewSocketClient(common.DefaultSocketPath())
	if err == nil {
		defer client.Close()
		resp, err := client.SendCommand(common.SocketCommand{Cmd: "status"})
		if err == nil && resp != nil {
...
func daemonList(ctx *cli.Context) error {
	...
	if err == nil {
		defer client.Close()
		resp, err := client.SendCommand(common.SocketCommand{Cmd: "list"})
		if err == nil && resp != nil {
			fmt.Println(string(resp))
			return nil
		}
